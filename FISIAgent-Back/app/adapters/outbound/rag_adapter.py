...
class RAGAdapter(RAGServicePort):
    ...
    def __init__(self, docs_path: str = "app/docs/fisi", persist_directory: str = "chroma_db", enabled: bool = True):
        """
        ...
        Args:
            ...
            enabled: Si es False, no carga el modelo de embeddings (~118M parámetros)
                     ni ChromaDB -- pensado para entornos con RAM limitada (ej. el free
                     tier de Render, 512MB, no alcanza para BETO + este modelo a la vez).
                     El chat sigue funcionando (BETO + Gemini), solo sin contexto RAG.
        """
        ...
        if not self.enabled:
            ...
            logger.warning(f"RAGAdapter deshabilitado por configuración (ENABLE_RAG=false). Docs: {self.docs_path}")
            return
    ...
    def load_documents(self):
        ...
        """
        if not self.enabled:
            logger.warning("RAG deshabilitado, se omite la carga de documentos.")
            return
        try:
            # 1. Cargar documentos
