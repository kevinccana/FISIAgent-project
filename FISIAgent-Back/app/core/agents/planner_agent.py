...
"""
    ...
    def can_handle(self, task: AgentTask) -> bool:
        ...
        return task.role == AgentRole.PLANNER
    ...
    def execute(self, task: AgentTask) -> AgentResult:
        ...
        try:
            ...
            if action == "suggest_priority":
                return self._suggest_task_priority(task.input_data)
            elif action == "analyze_schedule":
                return self._analyze_daily_schedule(task.input_data)
            elif action == "suggest_organization":
                return self._suggest_task_organization(task.input_data)
            else:
                return AgentResult(
                    success=False,
                    data=None,
    ...
    def _suggest_task_priority(self, data: dict) -> AgentResult:
        """
...
"""
        ...
        response = self.llm_service.generate_response(
            prompt=prompt,
            temperature=0.5  # Algo determinista
        )
        
        # Parsear respuesta
        suggestion = self._parse_priority_response(response, new_task)
        
        return AgentResult(success=True, data=suggestion)
    
    def _analyze_daily_schedule(self, data: dict) -> AgentResult:
        """
...
"""
    ...
    def _suggest_task_organization(self, data: dict) -> AgentResult:
        """
